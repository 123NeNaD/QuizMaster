...
File: brojispitanikaBarGraph.js
...
------------------------------------------------------------------------------*/
d3.csv("grafiktesta.csv", function(error,data) {
  if(error) console.log("Error: data not loaded!");

  /*----------------------------------------------------------------------------
  Convert data if necessary. We want to make sure our brojispitanika vaulues are
  ...
  data.forEach(function(d) {
    d.opseg = d.opseg;
    d.brojispitanika = +d.brojispitanika;       // try removing the + and see what the console prints
    console.log(d.brojispitanika);   // use console.log to confirm
  });
  
  // Specify the domains of the x and y scales
  xScale.domain(data.map(function(d) { return d.opseg; }) );
  yScale.domain([0, d3.max(data, function(d) { return d.brojispitanika; } ) ]);
  ...
  svg.selectAll('rect')
    ...
    .attr({
      "x": function(d) { return xScale(d.opseg); },
      "y": function(d) { return yScale(d.brojispitanika); },
      "width": xScale.rangeBand(),
      "height": function(d) { return  height - yScale(d.brojispitanika); }
    })
    .style("fill", function(d,i) { return 'rgb(20, 20, ' + ((i * 30) + 100) + ')'});
        ...
        svg.selectAll('text')
            ...
            .text(function(d){
                return d.brojispitanika;
            })
            .attr({
                "x": function(d){ return xScale(d.opseg) +  xScale.rangeBand()/2; },
                "y": function(d){ return yScale(d.brojispitanika)+ 12; },
