	...
	<body>
		...
		<div id="Container32">
					...
					<div style="width:32%; height:170px; background-color:#efceff; float:left; text-align:center;">
						<form name="mojaforma33" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
							<br>
							<fieldset>
							...
							<input type="submit" name="rezultati" value="POGLEDAJ REZULTATE" style="background-color:#34056d; color:white; cursor:pointer; position:relative; top:10px; left:34px;"> <br><br>
							...
							<?php
								...
								if(isset($_POST['rezultati'])){
									...
									$rezultat1 = mysqli_query($konekcija, $upit1) or die("Greska1: " . mysqli_error($konekcija));
									...
									if(mysqli_num_rows($rezultat1) == 1){
										...
										$rezultat3 = mysqli_query($konekcija, $upit3) or die("Greska2: " . mysqli_error($konekcija));
										if(mysqli_num_rows($rezultat3) == 1){
											
											$niz = mysqli_fetch_array($rezultat3);
											$_SESSION['tipankete'] = $niz['tip_ankete'];
											$_SESSION['imeankete'] = $_POST['imeankete'];
											header("Location: rezultatianketiranjadrugastranica.php");
										
										} else {
											echo "<label style='color:red;'><center><b>Izabrana anketa nije vasa! Mozete pogledati rezultate samo vasih anketa.</b></center></label>";
										}
									}
...
<?php	
	...
	$upit4 = "SELECT * FROM ankete WHERE autor_ankete = '".$_SESSION['kime']."'";
		
	$rezultat4 = mysqli_query($konekcija, $upit4)
		or die("Greska u upitu3: ".mysqli_errno($konekcija));
